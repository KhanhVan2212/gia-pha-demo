  ...
  const nameIndex = people
    ...
    .sort((a, b) => {
      if (a.generation !== b.generation) {
        return a.generation - b.generation; // Đời cao (số nhỏ) lên trước
      }

      // Cùng đời, ưu tiên người chính thống (isPatrilineal = true) lên trước
      if (a.isPatrilineal !== b.isPatrilineal) {
        return a.isPatrilineal ? -1 : 1;
      }

      // Cùng nhóm (chính thống hoặc không), ưu tiên nam (giả sử nam có gender = 1) lên trước nữ
      if (a.gender !== b.gender) {
        if (a.gender === 1) return -1;
        if (b.gender === 1) return 1;
      }

      // Cùng đời, cùng nhóm, cùng giới tính, sắp xếp theo tên
